...
public final class FixStreamMessageParserTest
{
    ...
    public void shouldParseMessageSegmentContainingNeitherCompleteStartPrefixNorEndSuffix()
    {
        ...
        // it doesn't contain enough data to be identified as either a message start nor a message end
    ...
    @Test
    public void shouldRecogniseTruncatedMessageLessThanBufferSize()
    {
        final byte[] truncatedExecutionReport = FixMessageUtil.getTruncatedExecutionReport();
        final byte[] executionReport = FixMessageUtil.getExecutionReport();

        ByteBuffer firstBuffer = ByteBuffer.wrap(truncatedExecutionReport);
        ...
        assertArrayEquals(truncatedExecutionReport, results.get(0));
        assertArrayEquals(executionReport, results.get(1));
        assertEquals(truncated[0], 1);
    }

    @Test
    public void shouldThrowErrorAndNotPublishWhenParsingMessageLongerThanMaxAllowedMessageSize()
    {
