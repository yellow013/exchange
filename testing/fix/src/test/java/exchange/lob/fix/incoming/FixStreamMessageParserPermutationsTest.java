...
public class FixStreamMessageParserPermutationsTest
{
    ...
    public void messageBoundaryConditions()
    {
        ...
        for (int i = 1; i <= 6; i++) // possible fragmentation of "8=FIX." prefix of message 1
        {
            for (int j = 1; j <= 15; j++) // possible fragmentation of "|10=067|8=FIX." block between messages 1 and 2
            {
                for (int k = 1; k <= 15; k++) // possible fragmentation of "|10=067|8=FIX." block between messages 2 and 3
                {
                    for (int l = 1; l <= 9; l++) // possible fragmentation of "|10=067|" suffix of message 3
                    {
    ...
    private void performIteration(final ByteBuffer bb, final List<byte[]> expectedMessages, final BytesToConsumeCalculator bytesConsumer)
    {
        ...
        try
        {
            ...
            assertThat(messages).matches(actualMessages ->
            {
                ...
                Iterator<byte[]> it1 = expectedMessages.iterator();
                final Iterator<? extends byte[]> it2 = actualMessages.iterator();
        ...
        {
            handleException(bb, offsetsUsedForIteration);
            throw e;
        }
        bb.clear();
    }

    private void handleException(ByteBuffer bb, List<Integer> offsetsUsedForIteration)
    {
